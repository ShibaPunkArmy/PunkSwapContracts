...
// File: contracts/interfaces/IPUNKCallee.sol
...
contract PUNKPool is IPUNKPool, ERC20Permit, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    IPUNKFactory public override factory;
    ...
    constructor() public ERC20Permit("PunkSwap LP", "PUNK-LP", "1") {
        factory = IPUNKFactory(msg.sender);
    }
    ...
    function initialize(
        ...
        require(msg.sender == address(factory), "PUNK: FORBIDDEN");
        token0 = _token0;
        token1 = _token1;
        ampBps = _ampBps;
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(balance0 >= data.reserve0 && balance1 >= data.reserve1, "PUNK: UNSYNC_RESERVES");
        ...
        require(amount0 > 0 || amount1 > 0, "PUNK: INSUFFICIENT_LIQUIDITY_BURNED");
    ...
    function swap(
        ...
        require(amount0Out > 0 || amount1Out > 0, "PUNK: INSUFFICIENT_OUTPUT_AMOUNT");
        (bool isAmpPool, ReserveData memory data) = getReservesData(); // gas savings
        require(
        ...
        ReserveData memory newData;
        {
            ...
            require(to != address(_token0) && to != address(_token1), "PUNK: INVALID_TO");
            ...
            if (callbackData.length > 0)
                IPUNKCallee(to).ksSwapCall(msg.sender, amount0Out, amount1Out, callbackData);
        ...
        uint256 amount1In = newData.reserve1 > data.reserve1 - amount1Out
            ...
            : 0;
        require(amount0In > 0 || amount1In > 0, "PUNK: INSUFFICIENT_INPUT_AMOUNT");
        verifyBalance(
            amount0In,
            amount1In,
    ...
    function name() public override view returns (string memory) {
        ...
        return string(abi.encodePacked("PunkSwap LP ", _token0.symbol(), "-", _token1.symbol()));
    }

    function symbol() public override view returns (string memory) {
        IERC20Metadata _token0 = IERC20Metadata(address(token0));
        IERC20Metadata _token1 = IERC20Metadata(address(token1));
        return string(abi.encodePacked("PUNK-LP ", _token0.symbol(), "-", _token1.symbol()));
    }
    ...
    function safeUint112(uint256 x) internal pure returns (uint112) {
        require(x <= MAX_UINT112, "PUNK: OVERFLOW");
        return uint112(x);
    }
}

// File: contracts/PUNKFactory.sol
...
contract PUNKFactory is IPUNKFactory {
    ...
    function createPool(
        ...
        require(address(token0) != address(0), "PUNK: ZERO_ADDRESS");
        require(ampBps >= BPS, "PUNK: INVALID_BPS");
        // only exist 1 unamplified pool of a pool.
        require(
            ampBps != BPS || getUnamplifiedPool[token0][token1] == address(0),
            "PUNK: UNAMPLIFIED_POOL_EXISTS"
        );
        require(feeUnits > 0 && feeOptions[feeUnits], "PUNK: FEE_OPTION_NOT_EXISTS");
        pool = address(new PUNKPool());
        PUNKPool(pool).initialize(token0, token1, ampBps, feeUnits);
    ...
    function setFeeConfiguration(address _feeTo, uint24 _governmentFeeUnits)
        ...
        onlyFeeSetter
    {
        require(_governmentFeeUnits > 0 && _governmentFeeUnits < 20000, "PUNK: INVALID FEE");
        feeTo = _feeTo;
    ...
    function enableFeeOption(uint24 _feeUnits) external override onlyFeeSetter {
        require(_feeUnits > 0, "PUNK: INVALID FEE");
